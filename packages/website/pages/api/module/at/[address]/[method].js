import pmABI from '@polly-tools/core/abi/PollyModule.json';
...
export default async (req, res) => {
    ...
    const data = {};
    const {address, method, ...query} = getQuery(req);

    if(abi.supportsMethod(method)){

        const provider = getProvider();
        const contract = new ethers.Contract(address, pmABI, provider);
