...
contract Meta is PollyModule {
    ...
    mapping(uint => mapping(string => string)) private _strings;
    mapping(uint => mapping(string => bool)) private _bools;
    mapping(uint => mapping(string => uint)) private _uints;

    mapping(uint => mapping(string => bool)) private _locked;


    modifier isUnlockedKey(uint id_, string memory key_) {
    ...
    /// JSON
    function getJSON(uint id_, JSONKey[] memory keys_) public view returns (string memory) {
    ...
    /// Access
    function lockKey(uint id_, string memory key_) public {
        _locked[id_][key_] = true;
    }

    function isLockedKey(uint id_, string memory key_) public view returns (bool) {
    ...
    function setString(uint id_, string memory key_, string memory value_) public isUnlockedKey(id_, key_) {
        _strings[id_][key_] = value_;
    }

    function setBool(uint id_, string memory key_, bool value_) public isUnlockedKey(id_, key_) {
        _bools[id_][key_] = value_;
    }

    function setInt(uint id_, string memory key_, uint value_) public isUnlockedKey(id_, key_) {
    ...
    function getInts(uint id_, string memory key_) public view returns (uint) {
