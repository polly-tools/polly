...
contract Nft is ERC721, PollyModule {
  ...
  function tokenUri(uint id_) public view returns (string memory) {
    ...
    if(getAddress('aux.tokenUri') != address(0)){
      return NftAux(getAddress('aux.tokenUri')).tokenUri(id_);
    }
    else
  ...
  function contractUri() public view returns (string memory) {
    if(getAddress('aux.contractUri') != address(0)){
      return NftAux(getAddress('aux.contractUri')).contractUri();
    }
    else {
  ...
  function _mintFor(address to_, uint id_) private {
    ...
    if(getAddress('aux.beforeMint') != address(0))
      id_ = NftAux(getAddress('aux.beforeMint')).beforeMint(id_, NftAux.Msg(msg.sender, msg.value, msg.data, msg.sig));
    ...
    if(getAddress('aux.afterMint') != address(0))
      NftAux(getAddress('aux.afterMint')).afterMint(id_, NftAux.Msg(msg.sender, msg.value, msg.data, msg.sig));
  ...
  function tokenMax() public view returns (uint) {
    if(getAddress('aux.tokenMax') != address(0)){
      return NftAux(getAddress('aux.tokenMax')).tokenMax();
    }
    else {
  ...
  function tokenPrice(uint id_) public view returns (uint) {
    if(getAddress('aux.tokenPrice') != address(0)){
      return NftAux(getAddress('aux.tokenPrice')).tokenPrice(id_);
    }
    else
  ...
  function tokenPublic(uint id_) public view returns (bool) {
    if(getAddress('aux.tokenPublic') != address(0)){
      return NftAux(getAddress('aux.tokenPublic')).tokenPublic(id_);
    }
    else
  ...
  function tokenRecipient(uint id_) public view returns (address) {
    if(getAddress('aux.tokenRecipient') != address(0)){
      return NftAux(getAddress('aux.tokenRecipient')).tokenRecipient(id_);
    }
    else
